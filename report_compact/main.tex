...
\tikzstyle{block} = [rectangle, rounded corners, draw=black!65, fill=blue!5,
  text width=7.6cm, align=center, minimum height=0.85cm, inner sep=4pt, font=\scriptsize]
\tikzstyle{decision} = [rectangle, rounded corners, draw=black!65, fill=orange!8,
  text width=7.6cm, align=center, minimum height=0.85cm, inner sep=4pt, font=\scriptsize]
\tikzstyle{frozen} = [rectangle, rounded corners, draw=black!65, fill=green!8,
  text width=7.6cm, align=center, minimum height=0.85cm, inner sep=4pt, font=\scriptsize]
\tikzstyle{arch-arrow} = [thick,-{Stealth[length=2mm]}]
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{model-comparison.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{artifact-lifecycle.png}
...
\node (n1) [block] {Raw dataset --- \texttt{merged\_severity\_dataset\_not\_keyword\_keyword.csv} (1{,}294 rows) $\rightarrow$ \texttt{dropna(content, severity)}};
\node (n2) [block, below=of n1] {Label mapping: severity 0 $\rightarrow$ No Crisis; 1 $\rightarrow$ Implicit Crisis; 2--6 $\rightarrow$ Explicit Crisis};
\node (n3) [decision, below=of n2] {Duplicate check on \texttt{content} (before split) --- 0 duplicate-content rows found, 0 removed (verified)};
\node (n4) [block, below=of n3] {Stratified 70/15/15 split, seed=42 $\rightarrow$ Train 905 / Val 194 / Test 195 --- zero row/text overlap};
\node (n5) [decision, below=of n4] {Validation-only hyperparameter search: 9 vectorizer configs (Stage A) + LR $C$/class-weight grid (Stage B), selected by validation macro-F1};
\node (n6) [frozen, below=of n5] {\textbf{Frozen config}: \texttt{char\_wb}, n-gram (3,5), min\_df=2, max\_df=0.9, sublinear\_tf, max\_features=50{,}000};
\node (n7) [block, below=of n6] {\texttt{fit\_transform(train)} $\rightarrow$ 17{,}708 features; \texttt{transform(val)}, \texttt{transform(test)} --- no refit};
\node (n8) [block, below=of n7] {LogisticRegression($C{=}1.0$, balanced, lbfgs, max\_iter=5000) \texttt{.fit(X\_train, train\_labels)}};
\node (n9) [decision, below=of n8] {\texttt{predict\_proba(X\_test)} $\rightarrow$ argmax $\rightarrow$ predicted class --- test set touched exactly once};
\node (n10) [block, below=of n9] {Metrics, confusion matrix, top features; artifacts saved (\texttt{.joblib}) and reload-verified};
\node (n11) [frozen, below=of n10] {\textbf{Deployment}: \texttt{backend/app.py} loads \texttt{.joblib} once at startup $\rightarrow$ \texttt{transform} + \texttt{predict\_proba} per request --- no training};
\draw[arch-arrow] (n1) -- (n2);
\draw[arch-arrow] (n2) -- (n3);
\draw[arch-arrow] (n3) -- (n4);
\draw[arch-arrow] (n4) -- (n5);
\draw[arch-arrow] (n5) -- (n6);
\draw[arch-arrow] (n6) -- (n7);
\draw[arch-arrow] (n7) -- (n8);
\draw[arch-arrow] (n8) -- (n9);
\draw[arch-arrow] (n9) -- (n10);
\draw[arch-arrow] (n10) -- (n11);
\end{tikzpicture}
\caption{TF-IDF + Logistic Regression: complete lifecycle from raw dataset to deployed inference, traced from \texttt{experiments/tfidf\_baseline/run\_tfidf\_baseline.py} and \texttt{backend/app.py}. Full detail in \texttt{New\_docs/architecture/tfidf\_architecture.md}.}
\end{figure}
...
\centering
\begin{tikzpicture}[node distance=0.16cm]
\node (b1) [block] {Pre-made \texttt{train/validation/test.csv} (content, label) $\rightarrow$ \texttt{dropna} + label sanity check ($\in\{0,1,2\}$)};
\node (b2) [block, below=of b1] {\texttt{BertTokenizerFast.from\_pretrained('bert-base-uncased')} --- truncation=True, padding='max\_length', max\_length=256};
\node (b3) [block, below=of b2] {Class weights (\texttt{compute\_class\_weight('balanced')}) from train labels only};
\node (b4) [block, below=of b3] {\texttt{BertForSequenceClassification.from\_pretrained('bert-base-uncased', num\_labels=3)}};
\node (b5) [decision, below=of b4] {\texttt{WeightedTrainer}: CrossEntropyLoss(weight=class\_weights); 8 epochs, lr=2e-5, batch 16/32, warmup$\approx$10\%, seed=42};
\node (b6) [block, below=of b5] {\texttt{trainer.train()}: forward $\rightarrow$ loss $\rightarrow$ backprop $\rightarrow$ AdamW step; best checkpoint by validation macro-F1; early stopping (patience=3)};
\node (b7) [decision, below=of b6] {\texttt{trainer.predict(test\_dataset)} $\rightarrow$ logits $\rightarrow$ argmax --- test set touched once};
\node (b8) [frozen, below=of b7] {\texttt{model.half()} (fp16) $\rightarrow$ \texttt{save\_pretrained()} $\rightarrow$ frozen artifact \texttt{models/bert\_no\_severity/}};
\node (b9) [frozen, below=of b8] {\textbf{Deployment}: \texttt{AutoTokenizer} / \texttt{AutoModelForSequenceClassification.from\_pretrained(local\_files\_only=True)}, loaded once at startup};
\node (b10) [block, below=of b9] {\texttt{predict\_bert(text)}: tokenizer(truncation=True, max\_length=256) $\rightarrow$ \texttt{model(**encoded).logits} under \texttt{torch.inference\_mode()}};
\node (b11) [block, below=of b10] {\texttt{softmax(logits, dim=-1)} $\rightarrow$ argmax $\rightarrow$ predicted class + confidence --- no training at inference};
\draw[arch-arrow] (b1) -- (b2);
\draw[arch-arrow] (b2) -- (b3);
\draw[arch-arrow] (b3) -- (b4);
\draw[arch-arrow] (b4) -- (b5);
\draw[arch-arrow] (b5) -- (b6);
\draw[arch-arrow] (b6) -- (b7);
\draw[arch-arrow] (b7) -- (b8);
\draw[arch-arrow] (b8) -- (b9);
\draw[arch-arrow] (b9) -- (b10);
\draw[arch-arrow] (b10) -- (b11);
\end{tikzpicture}
\caption{BERT: training path (top, \texttt{ipnyb/bert\_no\_severity.ipynb}) and frozen inference path (bottom, \texttt{backend/app.py}), explicitly distinguished. Full detail in \texttt{New\_docs/architecture/bert\_architecture.md}.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{dataset-evaluation.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{live-prediction-sequence.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{overall_metrics.png}
\caption{Overall accuracy, macro precision, macro recall, and macro F1, TF-IDF + LR vs.\ BERT.}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{per_class_f1.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{confidence_vs_correctness.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{implicit_crisis_metrics.png}
